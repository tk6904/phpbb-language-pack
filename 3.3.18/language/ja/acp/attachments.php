...
$lang = array_merge($lang, array(
	...
	'DOWNLOAD_REMOVE_IPS_EXPLAIN'	=> '複数のIPアドレスを削除（または除外）するにはマウスとキーボードをうまく活用してください（SHIFTキー または CTRLキー を押しながらクリック）。除外されたIPアドレスは背景が青くなります。',
	...
	'FILES_GONE'			=> '選択したファイルのいくつかは存在しません。既に削除されたものと思われます。存在していた添付情報は削除されました。',
	'FILES_STATS_WRONG'		=> 'ファイルの統計データが不正確かもしれないため再同期が必要です。実際の値: 添付ファイルの数 = %1$d, 添付ファイル全体のサイズ = %2$s<br />%3$s再同期%4$s してください。',
	...
	'IMAGE_QUALITY'				=> 'アップロード画像の添付ファイルの品質（JPEGのみ）',
	'IMAGE_QUALITY_EXPLAIN'		=> '50％（ファイルサイズが小さい）から 90％（品質が高い）の間の値を指定します。90％以上の品質になるとファイルサイズが大きくなり、無効になります。最大画像サイズが 0 × 0px 以外の値に設定されている場合にのみ適用されます。',
	'IMAGE_STRIP_METADATA'		=> '画像のメタデータを削除する（JPEGのみ）',
	'IMAGE_STRIP_METADATA_EXPLAIN'	=> '作者名、GPS座標、カメラの詳細などのExifメタデータを削除します。最大画像サイズが 0 × 0px 以外の値に設定されている場合にのみ適用されます。',
	...
	'MAX_IMAGE_SIZE_EXPLAIN'		=> '添付画像の最大サイズです。phpBB はアップロード前にこの最大サイズに合わせて画像を自動的にリサイズする場合があります。サイズチェックと自動リサイズを無効にするには、両方の値を 0 x 0 px に設定してください。',
	...
	'NO_EXT_GROUP'				=> 'なし',
	'NO_EXT_GROUP_ALLOWED_PM'	=> 'プライベートメッセージには、<a href="%s">許可されている拡張子グループ</a>がありません',
	'NO_EXT_GROUP_ALLOWED_POST'	=> '投稿には、<a href="%s">許可されている拡張子グループ</a>がありません',
	...
	'RESYNC_FILES_STATS_CONFIRM'	=> 'ファイル統計データを再同期してもよろしいですか？',
	...
	'UPLOAD_POST_NOT_EXIST'			=> 'ファイル「%1$s」は投稿番号 %2$d にアップロードできません。指定された投稿が存在しません。',
));
